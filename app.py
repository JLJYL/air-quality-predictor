...
TOL_MINUTES_PRIMARY = 15
TOL_MINUTES_FALLBACK = 120
...
def index():
    ...
    # ✅ 印出實際抓到的座標（即時）
    print(f"📍 [Live Coordinates] Current request location → latitude={TARGET_LAT}, longitude={TARGET_LON}")
    ...
    print("\n===============================")
    print("🌍 [STAGE 1] Station Information")
    print("===============================")
    ...
    print("\n===============================")
    print("📊 [STAGE 2] OpenAQ JSON → DataFrame")
    print("===============================")
    if not current_observation_raw.empty:
        ...
        print("\n".join(json_preview.splitlines()[:10]) + "\n... (truncated) ...")
    else:
    ...
    if not current_observation_raw.empty and LAST_OBSERVATION is not None and not LAST_OBSERVATION.empty:
        ...
        if all(col in observation_for_prediction.columns for col in FEATURE_COLUMNS):
             ...
             print("\n===============================")
        print("🧠 [STAGE 3] Model Input DataFrame")
        print("===============================")
        if is_valid_for_prediction:
            print(observation_for_prediction.head(3).to_string(index=False))
    else:
    ...
    if TRAINED_MODELS and POLLUTANT_PARAMS and is_valid_for_prediction and observation_for_prediction is not None:
        try:
            ...
            print("\n===============================")
            print("📈 [STAGE 4] Model Output (Predictions)")
            print("===============================")
            print(future_predictions[['datetime', 'aqi_pred']].head(10).to_string(index=False))
