...
def index():
   ...
   # ... (app.py 截斷)
    ...
    if not current_observation_raw.empty and LAST_OBSERVATION is not None and not LAST_OBSERVATION.empty:
        ...
        # 2. **核心修正**: 將當前觀測的時間設置為起始時間 (移除時區以匹配訓練集的特徵生成邏輯)
        #    注意：在 predict_future_multi 中我們會重新加上時區
        observation_for_prediction['datetime'] = dt_val

        # 3. **核心修正**: 用當前測站的觀測值覆蓋訓練時儲存的 "最新觀測值"
        for col in latest_row.index:
            # 覆蓋所有污染物、AQI，以及任何天氣欄位（如果存在）
            if col in observation_for_prediction.columns and not any(s in col for s in ['lag_', 'rolling_']):
                if col in POLLUTANT_TARGETS or col == 'aqi' or col in ['temperature', 'humidity', 'pressure']:
                    # 確保將 'pm25' 覆蓋到 'pm25_value' 的位置，這是 LAST_OBSERVATION 儲存的格式
                    col_to_match = f'{col}_value' if col in POLLUTANT_TARGETS else col
                    if col_to_match in observation_for_prediction.columns:
                         observation_for_prediction[col_to_match] = latest_row[col]
                    # 由於 LAST_OBSERVATION 中的 'aqi' 欄位名稱就是 'aqi'，這裡可以直接覆蓋
                    elif col == 'aqi':
                        observation_for_prediction['aqi'] = latest_row['aqi']
                    # 覆蓋 weather features (temperature, humidity, pressure)
                    elif col in ['temperature', 'humidity', 'pressure']:
                        observation_for_prediction[col] = latest_row[col]

        # 4. 進行額外檢查：如果當前觀測數據有任一污染物值，則用它來更新 LAG_1h 特徵
        #    這雖然不能完美解決滾動/多滯後問題，但能確保預測至少從當前的空氣品質值開始遞歸
        ...
        # 確保所有必要的特徵列都在，以便進入預測流程
        # 注意：雖然 LAG/ROLLING 特徵是舊的，但我們已經用最新的觀測值覆蓋了第 0 小時的輸入 (例如 pm25_value)
